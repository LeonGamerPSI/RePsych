...
class PauseSubState extends MusicBeatSubstate
{
	...
	override function create()
	{
		...
		var pauseSong:Dynamic = null;
		try
		{
			if (ClientPrefs.data.pauseMusic == 'Song')
			{
				if (PlayState.SONG.song != null)
					 pauseSong = Paths.inst(PlayState.SONG.song);
			}
			else
				 pauseSong = Paths.music(getPauseSong());
			if (pauseSong != null)
				pauseMusic.loadEmbedded(pauseSong, true, true);
		}
		catch (e:Dynamic)
		{
		}
