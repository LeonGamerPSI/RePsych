...
class CharacterEditorState extends MusicBeatState
{
	...
	function makeUIMenu()
	{
		...
		uibox = topBox.root.findElement('tabs');
		uibox.cameras = [camHUD];
		uibox.scrollFactor.set();
		uibox.setPosition(FlxG.width - 275, 25);
		add(uibox);

		var tickbox:Tickbox = uibox.findElement('highGhost');
		tickbox.onChange = (checked) ->
		{
		...
		var alphaSlider:Slider = uibox.findElement('ghostAlpha');
		alphaSlider.value = ghostAlpha;
		...
		var btn:Button = uibox.findElement('placeGhost');
		btn.onClick = remakeGhost;

		var reloadChar:Button = uibox.findElement('reloadCharBTN'),
			charText:TextInput = uibox.findElement('charNameTxt');
		reloadChar.onClick = function()
		{
			_char = charText.text;
			addCharacter(true);
			updatePointerPos();
		...
		var isPlayer:Tickbox = uibox.findElement('isPlayer');
		isPlayer.onChange = (plr) ->
		{
			character.isPlayer = plr;
		...
		UI_characterbox.x = uibox.x - 100;
		UI_characterbox.y = uibox.y + uibox.height;
	...
	{
		var tab_group = new FlxUI(null, null);
	...
	function reloadCharacterOptions()
	{
		...
		try
		{
			uibox.findElement('isPlayer').checked = character.isPlayer;
	...
	override function update(elapsed:Float)
	{
		...
		if (animationInputText.hasFocus || animationNameInputText.hasFocus || animationIndicesInputText.hasFocus || imageInputText.hasFocus
			|| healthIconInputText.hasFocus || vocalsInputText.hasFocus || ElementUtil.anythingFocused || ElementUtil.anythingFocused)
		{
